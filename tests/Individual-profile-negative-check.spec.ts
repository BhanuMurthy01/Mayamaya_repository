...
test('Profile picture upload - invalid file (>200KB) - Individual user', async ({ page }) => {

  // ✅ Pick latest Individual user dynamically
  const user = users
    .filter((u: any) => u.env === ENV && u.role === 'Individual')
    .sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];

  if (!user) {
    throw new Error(`❌ No Individual user found in ENV=${ENV}`);
  }
  ...
  // ✅ Validation (no navigation / no success)
